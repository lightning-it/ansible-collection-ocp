...
Agent-specific defaults use the `install_agent_*` prefix; the facade role continues to pass/consume the shared `ocp_install_*` variables.
...
- `install_agent_skip_download: true` reuses already staged `oc` client tools instead of downloading and unpacking them.
...
```yaml
---
- name: Use lit.ocp.install_agent
  hosts: all
  become: true
  roles:
